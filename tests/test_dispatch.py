...
def test_all_with_device():
    response = dm.dispatch.all(
        device_identifier=test_dispatch['device_identifier'])
    assert isinstance(response, dict)


@vcr.use_cassette('tests/cassettes/dispatch/push')
def test_push():
    response = dm.dispatch.push(
        test_dispatch['device_identifier'], test_dispatch['push_json'])
    assert isinstance(response, dict)


@vcr.use_cassette('tests/cassettes/dispatch/update')
def test_update():
    response = dm.dispatch.update(
        test_dispatch['device_identifier'], test_dispatch['update_oneshot_id'],
        test_dispatch['push_json'])
    assert isinstance(response, dict)


@vcr.use_cassette('tests/cassettes/dispatch/delete_with_id')
def test_delete_with_id():
    response = dm.dispatch.delete(
         test_dispatch['device_identifier'],
         dispatch_id=test_dispatch['delete_oneshot_id'])
