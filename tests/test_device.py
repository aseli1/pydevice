...
@vcr.use_cassette("fixtures/cassettes/device/all")
def test_all():
    response = dm.device.all()
    assert isinstance(response, str)  

@vcr.use_cassette("fixtures/cassettes/device/details")
def test_details():
    response = dm.device.details(test_device['details_id'])
    assert isinstance(response, str)

@vcr.use_cassette("fixtures/cassettes/device/approve")
def test_approve():
    response = dm.device.details(test_device['details_id'])
    assert isinstance(response, str)

@vcr.use_cassette("fixtures/cassettes/device/update")
def test_update():
    response = dm.device.update(test_device['update_id'], test_device['update_xml'])
    assert isinstance(response, str)

@vcr.use_cassette("fixtures/cassettes/device/delete")
def test_delete():
