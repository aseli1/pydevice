class Dispatch():
    ...
    def all(self, device_identifier=None):
        if device_identifier != None:
            url = self.device_base_url + "/" + device_identifier + "/dispatches.json"
        else:
    ...
    def delete(self, device_identifier, dispatch_id=None):
        if dispatch_id != None:
            url = self.device_base_url + "/" + device_identifier + "/dispatches/" + str(dispatch_id)
            request = self.r.delete(url)
        else:
            url = self.device_base_url + "/" + device_identifier + "/dispatches/destroy_all"
